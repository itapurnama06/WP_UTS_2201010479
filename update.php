...
if(isset($_POST['txUPDATE'])){
    ...
    $lahir = $_POST['txLAHIR'];
    if($pass1 == $pass2){
        $sql = "UPDATE tbmahasiswa SET nim = '$nim', nama = '$nama', alamat= '$alamat', tanggal_lahir = '$tgl', jenis_kelamin = '$jns', email= '$email', no_tlp = '$tlp', tempat_lahir='$lahir', passkey = '".md5($pass1)."' WHERE id = $iduser";
                ...
                <form method="post" action="">
                    ...
                    <div>
                        <label class="form-label"><b>NIM</b></label>
                            <input class="form-control" type="text" name="txNIM" required value="<?php echo $row['nim']; ?>">
                    </div>

                    <div>
                        <label class="form-label"><b>Nama</b></label>
                            <input class="form-control"  type="text" name="txNAMA" required  value="<?php echo $row['nama']; ?>">
                    </div>

                    <div>
                        <label class="form-label"><b>Alamat</b></label>
                            <input class="form-control"  type="text" name="txALAMAT" required  value="<?php echo $row['alamat']; ?>">
                    </div>  

                    <div>
                        <label class="form-label"><b>Tanggal Lahir</b></label>
                            <input class="form-control"  type="DATE" name="txTGL" required  value="<?php echo $row['tanggal_lahir']; ?>">
                    </div>

                    <div class="border border-2 mt-2">
                        <label class="form-label "><b>Jenis Kelamin</b></label><br>
                            <input type="radio" id="laki" name="txJNS" value="laki"
                            <?php echo $row['jenis_kelamin'] == 'laki' ? 'checked' : '' ?>>
                                <label class="form-label">Laki-Laki</label>
                            <input type="radio" id="perempuan" name="txJNS" value="perempuan" 
                            <?php echo $row['jenis_kelamin'] == 'perempuan' ? 'checked' : '' ?>>
                                <label class="form-label">Perempuan</label>
                    </div>

                    <div>
                        <label class="form-label"><b>Email</b></label>
                            <input class="form-control"  type="email" name="txEMAIL" required  value="<?php echo $row['email']; ?>">
                    </div>

                    <div>
                        <label class="form-label"><b>Nomor Telephone</b></label>
                            <input class="form-control"  type="number" name="txTELP" required  value="<?php echo $row['no_tlp']; ?>">
                    </div>

                    <div>
                        <label class="form-label"><b>Tempat Lahir</b></label>
                            <input class="form-control"  type="text" name="txLAHIR" value="<?php echo $row['tempat_lahir']; ?>">
                    </div>
